...
4. **系统重启或崩溃**
   ...
   - 如果问题持续存在，请减小缓冲区大小
   - 确保客户端不要发送过大的数据包（建议不超过2KB）
   - 检查供电是否稳定，不稳定的电源也可能导致系统重启

5. **ESP32在接收音频时重启**
   - 这通常是由于内存访问冲突或缓冲区溢出引起的
   - 已经通过添加内存保护和边界检查进行了优化
   - 确保客户端发送的数据大小不超过缓冲区大小（2KB）
   - 如果问题仍然存在，可以尝试降低采样率到16kHz
